...
(function (adguard, api) {
    ...
    const CssFilterRule = (function () {
        ...
        const parsePseudoClass = function (selector) {
            ...
            let nameEndIndex = adguard.utils.strings.indexOfAny(
                selector, [' ', '\t', '>', '(', '[', '.', '#', ':', '+', '~', '"', "'"],
                nameStartIndex + 1
            );
            if (nameEndIndex < 0) {
         ...
         */
        const constructor = function (rule, filterId) {
            api.FilterRule.call(this, rule, filterId);

            const mask = api.FilterRule.findRuleMarker(rule, CssFilterRule.RULE_MARKERS, CssFilterRule.RULE_MARKER_FIRST_CHAR);
            if (!mask) {
                throw new Error(`ruleText does not contain a CSS rule marker: ${rule}`);
            }

            const isInjectRule = CssFilterRule.INJECT_MARKERS.indexOf(mask) !== -1;
            ...
            const cssContent = rule.substring(indexOfMask + mask.length);
