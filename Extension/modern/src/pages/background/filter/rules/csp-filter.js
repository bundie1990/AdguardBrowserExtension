...
(function (adguard, api) {
    ...
    function putWithPriority(rule, whiteListRule, map) {
        const { cspDirective } = rule;
        const existRule = map[cspDirective];

        const pr1 = getRulePriority(rule);
        const pr2 = getRulePriority(whiteListRule);
        const pr3 = getRulePriority(existRule);

        const max = Math.max(pr1, pr2, pr3);
        if (max === pr1) {
    ...
    api.CspFilter = function (rules, badFilterRules) {
        const cspWhiteFilter = new api.UrlFilterRuleLookupTable();
        const cspBlockFilter = new api.UrlFilterRuleLookupTable();
        ...
        function getRules() {
            const rules = cspWhiteFilter.getRules();
            return rules.concat(cspBlockFilter.getRules());
        }

        /**
         * Searches for CSP rules matching specified request.
         * It worth noting that all (blocked and whitelisted!) CSP rules will be returned:
         * client should select which CSP rules will be added to headers.
         * @param url URL
        ...
        function findCspRules(url, documentHost, thirdParty, requestType) {
             ...
             * If a main_frame is loaded we should match rules without $SUBDOCUMENT
             * modifier (or with negation $~SUBDOCUMENT)
             * So if we pass `adguard.RequestTypes.OTHER` we won't match rules
             * with $SUBDOCUMENT modifier, as we expected
             *
             * For example:
             * rule1 = '||$csp'
            ...
            const whiteRules = cspWhiteFilter.findRules(url, documentHost, thirdParty, requestType, badFilterRules);

            const whitelistedRulesByDirective = Object.create(null);
            ...
            const blockingRules = cspBlockFilter.findRules(url, documentHost, thirdParty, requestType, badFilterRules);

            const rulesByDirective = Object.create(null);

            // Collect whitelist and blocking CSP rules in one array
            if (blockingRules) {
                for (i = 0; i < blockingRules.length; i += 1) {
                    rule = blockingRules[i];
                    const whiteListRule = whitelistedRulesByDirective[rule.cspDirective];
                    putWithPriority(rule, whiteListRule, rulesByDirective);
                }
            }

            const cspRules = [];
            Object.keys(rulesByDirective).forEach((key) => {
