...
(function (adguard, api) {
     ...
     */
    const DomainsLookupTable = function (rules) {
    ...
    DomainsLookupTable.prototype = {
        ...
        addRule(rule) {
            ...
            for (let i = 0; i < permittedDomains.length; i += 1) {
                const domainName = permittedDomains[i];
                let rules = this.lookupTable[domainName];
                if (!rules) {
                    rules = [];
        ...
        removeRule(rule) {
            ...
            for (let i = 0; i < permittedDomains.length; i += 1) {
                const domainName = permittedDomains[i];
                const rules = this.lookupTable[domainName];
                if (rules) {
        ...
        lookupRules(domainName) {
            ...
            for (let i = parts.length - 2; i >= 0; i -= 1) {
                host = `${parts[i]}.${host}`;
                const rules = this.lookupTable[host];
        ...
        getRules() {
            let result = [];
            for (const r in this.lookupTable) {
                const value = this.lookupTable[r];
                if (value) {
