...
(function (adguard, api) {
    ...
    CssFilter.prototype = {
        ...
        removeRule(rule) {
            ...
            this.exceptionRules = this.exceptionRules.filter((r) => r.ruleText !== ruleText);
            this.extendedCssRules = this.extendedCssRules.filter((r) => r.ruleText !== ruleText);
            this.domainSensitiveRules = this.domainSensitiveRules.filter((r) => r.ruleText !== ruleText);
            this.commonRules = this.commonRules.filter((r) => r.ruleText !== ruleText);
        ...
        buildCss(domainName, options) {
            ...
            const stylesheets = this._createCssStylesheets(rules);
            if (!genericHide && retrieveTraditionalCss) { // ExtCss rules are not contained in commonRules
                let commonCss = this._getCommonCss();
                if (cssInjectionOnly) {
                    commonCss = this._buildCssByRules(this.commonRules.filter((rule) => rule.isInjectRule));
                }
        ...
        _filterRules(domainName, options) {
            ...
            if (retrieveTraditionalCss && this.domainSensitiveRules !== null) {
                let iDomainSensitive = this.domainSensitiveRules.length;
            ...
            if (retrieveExtCss && this.extendedCssRules !== null) {
                let iExtendedCss = this.extendedCssRules.length;
                // eslint-disable-next-line no-plusplus
                while (iExtendedCss--) {
        ...
        _createCssStylesheets(rules) {
            const extendedCssRules = rules.filter((rule) => rule.extendedCss);

            const cssRules = rules.filter((rule) => !rule.extendedCss);
        ...
        _applyExceptionRules() {
            ...
            for (i = 0; i < this.domainSensitiveRules.length; i += 1) {
                ...
                if (exceptionRules) {
                    for (j = 0; j < exceptionRules.length; j += 1) {
            ...
            for (i = 0; i < this.commonRules.length; i += 1) {
            ...
            this.commonRules = this.commonRules.filter((el) => !(el.ruleText in newDomainSensitiveRulesMap));
        },
        ...
        _rollbackExceptionRule(exceptionRule) {
            ...
            this.domainSensitiveRules = this.domainSensitiveRules.filter((el) => !(el.ruleText in newCommonRulesMap));
        },
