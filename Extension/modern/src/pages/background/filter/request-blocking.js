...
adguard.webRequestService = (function (adguard) {
    ...
    const processGetSelectorsAndScripts = function (tab, documentUrl, cssFilterOptions, retrieveScripts) {
        ...
        const retrieveSelectors = !elemHideFlag
            && (cssFilterOptions & (CssFilter.RETRIEVE_TRADITIONAL_CSS + CssFilter.RETRIEVE_EXTCSS)) !== 0;
     ...
     * @returns {*|boolean}
     */
    const isRequestBlockedByRule = function (requestRule) {
        return requestRule
    ...
    const getCspRules = function (tab, requestUrl, referrerUrl, requestType) {
        ...
        // @@||example.org^$document or @@||example.org^$urlblock — disables all the $csp rules on all
        // the pages matching the rule pattern.
        const whitelistRule = adguard.requestFilter.findWhiteListRule(
            requestUrl,
            referrerUrl,
            adguard.RequestTypes.DOCUMENT
     ...
     */
    const processRequestResponse = function (tab, requestUrl, referrerUrl, requestType, responseHeaders) {
        if (requestType === adguard.RequestTypes.DOCUMENT) {
