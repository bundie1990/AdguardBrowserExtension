...
adguard.backend = (function (adguard) {
    ...
    const settings = {
        ...
        // This url is used in integration mode.
        // Adguard for Windows/Mac/Android intercepts requests to injections.adguard.com host.
    ...
    const adguardAppAddRule = function (ruleText, successCallback, errorCallback) {
        executeRequestAsync(
            `${settings.adguardAppUrl}type=add&rule=${encodeURIComponent(ruleText)}`,
            'text/plain',
            successCallback,
            errorCallback
        );
    ...
    const adguardAppRemoveRule = function (ruleText, successCallback, errorCallback) {
        executeRequestAsync(
            `${settings.adguardAppUrl}type=remove&rule=${encodeURIComponent(ruleText)}`,
            'text/plain',
            successCallback,
            errorCallback
        );
    ...
    const adguardAppAddRuleOld = function (ruleText, successCallback, errorCallback) {
        executeRequestAsync(
            `${settings.adguardAppUrlOld}type=add&rule=${encodeURIComponent(ruleText)}`,
            'text/plain',
            successCallback,
            errorCallback
        );
    ...
    const isAdguardAppRequest = function (requestUrl) {
        return requestUrl && (
            requestUrl.indexOf('/adguard-ajax-crossdomain-hack/') > 0
            || requestUrl.indexOf('/adguard-ajax-api/') > 0
        );
    };
